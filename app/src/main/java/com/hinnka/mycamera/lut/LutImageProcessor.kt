...
class LutImageProcessor {
    ...
    private var bitmapDenoisePreconditionY0U0V0Program = 0
    ...
    suspend fun applyLut(
        ...
        if (noiseReduction > 0) {
            renderBitmapDenoiseProfile(chromaDenoisedTexId, width, height, noiseReduction, denoiseAlgorithm)
            currentCoroutineContext().ensureActive()
        }
...
//        tempBitmap.recycle()
    ...
    private fun initBitmapDenoiseProfilePrograms() {
        ...
        bitmapDenoisePreconditionY0U0V0Program = compileComputeProgram(DenoiseProfileShaders.PRECONDITION_Y0U0V0, "BitmapDenoise_PreconditionY0U0V0")
        ...
        bitmapDenoiseBacktransformProgram = compileComputeProgram(DenoiseProfileShaders.BACKTRANSFORM_Y0U0V0, "BitmapDenoise_BacktransformY0U0V0")
        ...
        PLog.d(
            ...
            "Bitmap denoiseprofile programs initialized: pre=$bitmapDenoisePreconditionProgram " +
                "preY=$bitmapDenoisePreconditionY0U0V0Program decompose=$bitmapDenoiseDecomposeProgram " +
                "synth=$bitmapDenoiseSynthesizeProgram reduce1=$bitmapDenoiseReduceFirstProgram " +
                "reduce2=$bitmapDenoiseReduceSecondProgram back=$bitmapDenoiseBacktransformProgram " +
    ...
    private fun renderBitmapDenoiseProfile(
        ...
        if (!isBitmapDenoiseProfileReady(denoiseAlgorithm)) {
            renderTexturePassthrough(sourceTextureId, bitmapDenoiseFboId[1], width, height)
            return
        }
        ...
        when (denoiseAlgorithm) {
            DenoiseAlgorithm.Fast -> renderBitmapDenoiseProfileFast(sourceTextureId, width, height, params)
            DenoiseAlgorithm.HighQuality -> renderBitmapDenoiseProfileHighQuality(sourceTextureId, width, height, params)
        }
        checkGlError("renderBitmapDenoiseProfile")
    }

    private fun renderBitmapDenoiseProfileHighQuality(
        sourceTextureId: Int,
        width: Int,
        height: Int,
    ...
    private fun renderBitmapDenoiseProfileFast(
        sourceTextureId: Int,
        width: Int,
        height: Int,
        params: BitmapDenoiseParams
    ...
    private fun isBitmapDenoiseProfileReady(denoiseAlgorithm: DenoiseAlgorithm): Boolean {
        val highQualityReady = bitmapDenoisePreconditionProgram != 0 &&
            bitmapDenoiseNlmInitProgram != 0 &&
            bitmapDenoiseNlmFusedAccuProgram != 0 &&
            bitmapDenoiseNlmFinishProgram != 0
        val fastReady = bitmapDenoisePreconditionY0U0V0Program != 0 &&
        ...
        val algorithmReady = when (denoiseAlgorithm) {
            DenoiseAlgorithm.Fast -> fastReady
            DenoiseAlgorithm.HighQuality -> highQualityReady
        }
        return algorithmReady && bitmapDenoisePassthroughProgram != 0
    }

    private fun setupBitmapDenoiseResources(width: Int, height: Int) {
        for (i in bitmapDenoiseDetailTexIds.indices) {
            if (bitmapDenoiseDetailTexIds[i] != 0) {
                GLES30.glDeleteTextures(1, intArrayOf(bitmapDenoiseDetailTexIds[i]), 0)
        ...
        bitmapDenoiseReduceSize = kotlin.math.min(
            ...
            roundUp(bitmapDenoiseReduceBufSize, DenoiseProfileShaders.REDUCE_SECOND_LOCAL_X) /
                DenoiseProfileShaders.REDUCE_SECOND_LOCAL_X
        ).coerceAtLeast(1)

        if (bitmapDenoiseReduceMBufferId != 0) {
            GLES31.glDeleteBuffers(1, intArrayOf(bitmapDenoiseReduceMBufferId), 0)
            bitmapDenoiseReduceMBufferId = 0
        }
        if (bitmapDenoiseReduceRBufferId != 0) {
            GLES31.glDeleteBuffers(1, intArrayOf(bitmapDenoiseReduceRBufferId), 0)
            bitmapDenoiseReduceRBufferId = 0
        }
        val reduceBuffers = IntArray(2)
        GLES31.glGenBuffers(2, reduceBuffers, 0)
        bitmapDenoiseReduceMBufferId = reduceBuffers[0]
        bitmapDenoiseReduceRBufferId = reduceBuffers[1]
        GLES31.glBindBuffer(GLES31.GL_SHADER_STORAGE_BUFFER, bitmapDenoiseReduceMBufferId)
        GLES31.glBufferData(
            GLES31.GL_SHADER_STORAGE_BUFFER,
            bitmapDenoiseReduceBufSize * 4 * 4,
            null,
    ...
    private fun buildBitmapDenoiseParams(
        ...
        val fastScale = (strength * compensateStrength * scale).coerceAtLeast(1e-10f)
        val toYScaled = FloatArray(9)
        val toRgbScaled = FloatArray(9)
        for (i in 0 until 9) {
            toYScaled[i] = toY[i] / fastScale
            toRgbScaled[i] = toRgbBase[i] * fastScale
        }
    ...
    private fun dispatchBitmapDenoisePrecondition(
        input: Int,
        output: Int,
        width: Int,
        height: Int,
        ...
        GLES31.glUseProgram(bitmapDenoisePreconditionY0U0V0Program)
        bindComputeSampler(bitmapDenoisePreconditionY0U0V0Program, "uInput", 0, input)
        GLES31.glBindImageTexture(1, output, 0, false, 0, GLES31.GL_WRITE_ONLY, GLES31.GL_RGBA16F)
        setBitmapDenoiseCommonUniforms(bitmapDenoisePreconditionY0U0V0Program, width, height, params, params.fastWb)
        GLES31.glUniformMatrix3fv(
            GLES31.glGetUniformLocation(bitmapDenoisePreconditionY0U0V0Program, "uToY0U0V0"),
            1,
            false,
            matrix3ToGl(params.toY0U0V0),
            0
        )
        dispatchBitmapDenoiseImage(width, height, "BitmapDenoise fast precondition")
    }

    private fun dispatchBitmapDenoiseDecompose(
        input: Int,
        coarse: Int,
        detail: Int,
        width: Int,
        ...
        GLES31.glUniform1f(
            GLES31.glGetUniformLocation(bitmapDenoiseDecomposeProgram, "uInvSigma2"),
            1.0f / (sigmaBand * sigmaBand)
        )
        dispatchBitmapDenoiseImage(width, height, "BitmapDenoise fast decompose $scale")
    }

    private fun dispatchBitmapDenoiseSynthesize(
        coarse: Int,
        detail: Int,
        output: Int,
        width: Int,
        ...
        dispatchBitmapDenoiseImage(width, height, "BitmapDenoise fast synthesize")
    }

    private fun dispatchBitmapDenoiseBacktransform(
        input: Int,
        output: Int,
        width: Int,
        height: Int,
        ...
        GLES31.glBindImageTexture(1, output, 0, false, 0, GLES31.GL_WRITE_ONLY, GLES31.GL_RGBA16F)
        setBitmapDenoiseCommonUniforms(bitmapDenoiseBacktransformProgram, width, height, params, params.fastWb)
        GLES31.glUniform1f(
            GLES31.glGetUniformLocation(bitmapDenoiseBacktransformProgram, "uBias"),
            params.bias - 0.5f * ln(params.scale)
        )
        GLES31.glUniformMatrix3fv(
            GLES31.glGetUniformLocation(bitmapDenoiseBacktransformProgram, "uToRgb"),
            1,
            false,
            matrix3ToGl(params.toRgb),
            0
        )
        dispatchBitmapDenoiseImage(width, height, "BitmapDenoise fast backtransform")
    }

    private fun computeBitmapDenoiseThreshold(
        textureId: Int,
        width: Int,
        height: Int,
        scale: Int,
    ...
    private fun setBitmapDenoiseCommonUniforms(
        program: Int,
        width: Int,
        height: Int,
        params: BitmapDenoiseParams,
    ...
    fun release() {
        ...
        if (bitmapDenoisePreconditionY0U0V0Program != 0) GLES31.glDeleteProgram(bitmapDenoisePreconditionY0U0V0Program)
        ...
        bitmapDenoiseReduceBufSize = 0
        bitmapDenoiseReduceSize = 0
        if (bitmapDenoiseNlmU2BufferId != 0) {
            GLES31.glDeleteBuffers(1, intArrayOf(bitmapDenoiseNlmU2BufferId), 0)
            bitmapDenoiseNlmU2BufferId = 0
        }
